...
void myntuple::Loop()
{
	...
	std::cout<<"Number of events after selection on pT = "<<CutFlowP<<std::endl;
	std::cout<<"Number of events after selection on Eta = "<<CutFlowEta<<std::endl;
	std::cout<<"Number of events after selection on di-muon charge = "<<CutFlowCh<<std::endl;
	std::cout<<"Number of events after selection on di-muon mass = "<<CutFlowM<<std::endl;
	...
	for (Long64_t jentry=0; jentry<nentries;jentry++) {
		...
		for (unsigned int myFourMuIdx = 0; myFourMuIdx < nMyFourMuon; myFourMuIdx++) {
			...
			{
				CutFlow4Mu = CutFlow4Mu + 1;
				if((*MyFourMuonVtxCL)[myFourMuIdx] >= 0.005)// requiring all muons to come from the same vertex
				{
					CutFlowVertex = CutFlowVertex + 1;
					if((TrigThreeMuonJpsi3p5mu2 || TrigThreeMuonJpsi))// Selection on the trigger 
					{
						CutFlowTrig = CutFlowTrig + 1;
						if((rawMup4vect[0].Pt()>=2 && rawMup4vect[1].Pt()>=2 && rawMup4vect[2].Pt()>=2 && rawMup4vect[3].Pt()>=2))// Require a minimum momentum selection
						{
							CutFlowP = CutFlowP + 1;
