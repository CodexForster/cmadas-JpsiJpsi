...
void null_BWAll(){
  ...
  double massTh2Init = 6.6e+00, massTh2Min = 6.50, massTh2Max = 6.80;
  RooRealVar massTh2("massTh2", "massTh2", massTh2Init, massTh2Min, massTh2Max);
  massTh2.setConstant(kFALSE); //m_0
  double widthTh2Init = 0.1, widthTh2Min = 0.00, widthTh2Max = 1.50;
  ...
  double massTh3Init = 7.0e+00, massTh3Min = 6.80, massTh3Max = 7.20;
  RooRealVar massTh3("massTh3", "massTh3", massTh3Init, massTh3Min, massTh3Max);
  massTh3.setConstant(kFALSE); //m_0
  double widthTh3Init = 0.1, widthTh3Min = 0.00, widthTh3Max = 1.50;
  ...
  double massTh4Init = 7.3e+00, massTh4Min = 7.20, massTh4Max = 7.40;
  RooRealVar massTh4("massTh4", "massTh4", massTh4Init, massTh4Min, massTh4Max);
  massTh4.setConstant(kFALSE); //m_0
  double widthTh4Init = 0.15, widthTh4Min = 0.00, widthTh4Max = 1.50;
...
//   double massTh5Init = 9.6e+00, massTh5Min = 9.40, massTh5Max = 9.80;
//   RooRealVar massTh5("massTh5", "massTh5", massTh5Init, massTh5Min, massTh5Max);
//   massTh5.setConstant(kFALSE); //m_0
//   double widthTh5Init = 0.1, widthTh5Min = 0.00, widthTh5Max = 1.50;
//   RooRealVar widthTh5("widthTh5", "widthTh5", widthTh5Init, widthTh5Min, widthTh5Max);
//   widthTh5.setConstant(kFALSE); //Gamma_0
//   double LTh5Init = 0; // We assume S-wave
//   RooRealVar LTh5("LTh5", "LTh5", LTh5Init);
//   LTh5.setConstant(kTRUE);
//   double dTh5Init = 3.00, dTh5Min = 1.00, dTh5Max = 5.00;
//   RooRealVar dTh5("dTh5", "dTh5", dTh5Init, dTh5Min, dTh5Max);
//   dTh5.setConstant(kTRUE); 
//   // the parameters below are related to interference terms. 
//   // We set them to a constant value as we are not considering interference
//   double coefTh5Init = 1, coefTh5Min = 0, coefTh5Max = 1000;
//   RooRealVar coefTh5("coefTh5", "coefTh5", coefTh5Init, coefTh5Min, coefTh5Max);
//   coefTh5.setConstant(kTRUE);
//   double phiTh5Init = 0, phiTh5Min = -PI, phiTh5Max = PI;
//   RooRealVar phiTh5("phiTh5", "phiTh5", phiTh5Init, phiTh5Min, phiTh5Max);
//   phiTh5.setConstant(kTRUE);

//   // Here we construct our function with the parameters defined above
//   MyRelBWSquare Th5("Th5", "Th5", mx,
//       massTh5, widthTh5, LTh5, dTh5, coefTh5, phiTh5);
//   // We also need to take into account detector smearing which affect the resolution of the peak
...
//   RooArgList pdfList(dpsPdf, spsPdf, Th1Reso, Th2Reso, Th3Reso, Th4Reso, Th5Reso);
...
//   RooArgList numList(numDps, numSps, numTh1, numTh2, numTh3, numTh4, numTh5);
  ...
  */
  ...
  model.plotOn(frame, Components(Th2Reso), Name("Th2"), LineColor(kBlue), LineStyle(kDotted));
  model.plotOn(frame, Components(Th3Reso), Name("Th3"), LineColor(kRed), LineStyle(kDashDotted));
  model.plotOn(frame, Components(Th4Reso), Name("Th4"), LineColor(kGreen+1), LineStyle(kDotted));
//   model.plotOn(frame, Components(Th5Reso), Name("Th5"), LineColor(kBlack), LineStyle(kDashDotted));
...
//   leg.AddEntry(frame->findObject("Th5"), "BW4", "l");
  ...
  pullFrame->Draw();
  c1.Update();
  c1.SaveAs("figure/fit_null_BWAll.pdf");
